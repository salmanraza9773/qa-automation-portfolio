...
export default function Home() {
    ...
    fetch('/api/get-portfolio')
      ...
      .catch((err) => {
        ...
        setPortfolioData({
          profile: { 
            name: "Salman Raza", 
            role: "Software Testing & QA Automation Engineer", 
            bio: "Specializing in designing robust, scalable automation frameworks and ensuring enterprise system reliability across web, mobile, and API layers." 
          },
          skills: [],
          experience: [],
          projects: []
  ...
  return (
    <div className="bg-[#0B0F19] min-h-screen relative overflow-hidden">
      <Head>
        <title>{portfolioData?.profile?.name || "Salman Raza"} | Portfolio</title>
      ...
      {/* Cleaned Top Navigation Menu Bar */}
      <nav className="sticky top-0 z-50 bg-[#0d1322]/80 backdrop-blur-md border-b border-gray-800/80 px-6 py-4 flex justify-between items-center">
        {/* Left Side: Clean Professional Logo Badge */}
        <div className="flex items-center gap-2">
          <span className="font-mono font-bold tracking-wider text-white text-sm flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-cyan-400"></span>
            {portfolioData?.profile?.name?.toUpperCase() || "SALMAN RAZA"}
            <span className="text-cyan-400/90 text-xs font-normal border border-cyan-500/30 bg-cyan-950/40 px-2 py-0.5 rounded-full">
              QA ENGINE
            </span>
          </span>
        </div>

        {/* Right Side: Admin Shell Gate Only (Analytics Completely Removed for Public View) */}
        <div className="flex items-center gap-4">
          {isAdmin ? (
            <button onClick={handleLogout} className="text-xs font-mono bg-red-950/60 hover:bg-red-900/80 text-red-400 px-3 py-1.5 rounded-lg border border-red-900/50 transition-colors">
              Exit Session Gate
            </button>
          ) : (
            <button onClick={() => setShowAuthModal(true)} className="text-xs font-mono text-gray-400 hover:text-white transition-colors flex items-center gap-1.5">
              🔒 Admin Shell
            </button>
          )}
        </div>
